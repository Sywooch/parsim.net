<?php
  ...
  $menuItems=[];
  $options=[];
  foreach ($model->infoPosts as $post) {
      if(strtolower($post->category->alias)==strtolower($categoty)){
        $options=['class'=>'active'];
      }else{
        $options=[];
      }
      $menuItems[]=[
        'label'=>ucfirst(Yii::t('app',$post->category->alias)),
        'url'=>$model->getInfoUrl($post->category->alias),
        'options'=>$options
      ];
  }
      ...
      <div class="col-sm-12 col-fixed-content">
        <div class="row">
          ...
          <div class="col-md-9 col-sm-8">
            <?= $this->render('view',[
              'model'=>$model->getInformationPostByAlias($categoty)
            ]); ?>
          </div>
        </div>
      </div>
